...
public class Ghyslain : MonoBehaviour {
	...
	private List<GameObject> sphereInRangeList;		//All spheres that are in range but not pulled in Ghyslain gravity
	...
	void Start () {
		...
		sphereInRangeList = new List<GameObject>();
		...
		for (int i=0; i<sphereNb; i++) {
			...
			if (Random.Range(-1,1) < 0){
				uneSphere.rigidbody.velocity = new Vector3(0,0,400/iniSpherePos);
			}
			else{
				uneSphere.rigidbody.velocity = new Vector3(0,0,-400/iniSpherePos);
			}
...
*/
			...
			if (relDist.magnitude > sphereList[i].maxDist){
